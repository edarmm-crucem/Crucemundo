    ...
    for idx, file in enumerate(spreadsheets, start=1):
        ...
        try:
            ...
            if "CVC Agencias" not in titles:
                yield {"type": "skip", "msg": f"⤼ Sin hoja *CVC Agencias* → `{spreadsheet_name}`"}
                continue
            ...
            yield {"type": "status", "msg": f"📄 Generando PDF de la hoja **CVC Agencias**..."}
...
with row2col1:
    ...
    st.button("Próximamente", key="btnnextcardfuture", disabled=True)
    st.markdown("</div></div>", unsafe_allow_html=True)
