...
defaults = {
    ...
    "cvcagencias_status_lines": [],
    "cvcagencias_last_checked": "",
    "cvcagencias_total_files": 0,
    "cvcagencias_current_index": 0,
}
...
def clear_cvcagencias_state():
    for k in [
        "cvcagencias_locator",
        "cvcagencias_result",
        "cvcagenciaslocatorwidget",
        "cvcagencias_status_lines",
        "cvcagencias_last_checked",
        "cvcagencias_total_files",
        "cvcagencias_current_index",
        "cvcagencias_log",
    ]:
        st.session_state.pop(k, None)
...
def close_all_panels():
    ...
    st.session_state["opencvcagenciasform"] = False
...
def open_panel(panelname):
    ...
    elif panelname == "cvcagencias":
        clear_salida_state()
        clear_crucero_state()
        clear_agencia_state()
        clear_cvcfit_state()
        clear_cvcagencias_state()
        st.session_state["opencvcagenciasform"] = True
...
def do_logout():
    keys_to_delete = [
        ...
        "openbuscaragenciaform", "opencvcfitform", "opencvcagenciasform",
        ...
        "cvcfit_status_lines", "cvcfit_last_checked", "cvcfit_total_files", "cvcfit_current_index", "cvcfit_log",
        "cvcagencias_locator", "cvcagencias_result", "cvcagenciaslocatorwidget",
        "cvcagencias_status_lines", "cvcagencias_last_checked", "cvcagencias_total_files", "cvcagencias_current_index", "cvcagencias_log",
    ]
...
def add_cvcfit_status(message):
    ...
    lines.append(f"{datetime.now().strftime('%H:%M:%S')} Â· {message}")
    st.session_state["cvcfit_status_lines"] = lines[-15:]


def add_cvcagencias_status(message):
    lines = st.session_state.get("cvcagencias_status_lines", [])
    lines.append(f"{datetime.now().strftime('%H:%M:%S')} Â· {message}")
    st.session_state["cvcagencias_status_lines"] = lines[-15:]
...
def build_cvc_fit_pdf_from_locator(locator):
    ...
    yield {"type": "status", "msg": "ðŸ” Listando spreadsheets en el folder CVC Fit..."}
    ...
    yield {"type": "status", "msg": f"ðŸ“ Encontrados **{total}** spreadsheets. Iniciando bÃºsqueda del localizador `{locator_clean}`..."}
    ...
    for idx, file in enumerate(spreadsheets, start=1):
        ...
        yield {
            ...
            "msg": f"Revisando **{idx}/{total}** Â· `{spreadsheet_name}`",
        }
        ...
        try:
            ...
            if "Booking ES" not in titles:
                yield {"type": "skip", "msg": f"â¤¼ Sin hoja *Booking ES* â†’ `{spreadsheet_name}`"}
                continue

            if "CVC Fit" not in titles:
                yield {"type": "skip", "msg": f"â¤¼ Sin hoja *CVC Fit* â†’ `{spreadsheet_name}`"}
                continue

            g11_value = first_line(get_single_cell(spreadsheet_id, "Booking ES", "G11"))
            if str(g11_value).strip() != locator_clean:
                yield {"type": "skip", "msg": f"â¤¼ G11=`{g11_value}` â†’ no coincide en `{spreadsheet_name}`"}
                continue

            yield {"type": "status", "msg": f"âœ… **Â¡Coincidencia encontrada!** en `{spreadsheet_name}` â€” Leyendo nombre del pasajero..."}
            ...
            yield {"type": "status", "msg": f"ðŸ“„ Generando PDF de la hoja **CVC Fit**..."}
            ...
            yield {"type": "error", "msg": f"âš ï¸ Error en `{spreadsheet_name}`: {str(e)}"}
            continue

    raise Exception("No se ha encontrado el localizador en Booking ES!G11 de ningÃºn Sheet del folder.")


def build_cvc_agencias_pdf_from_locator(locator):
    locator_clean = str(locator).strip()
    if not locator_clean:
        raise Exception("Debes introducir un localizador.")
    ...
    for idx, file in enumerate(spreadsheets, start=1):
        ...
        try:
            ...
            if "CVC Agencias" not in titles:
                yield {"type": "skip", "msg": f"â¤¼ Sin hoja *CVC Agencias* â†’ `{spreadsheet_name}`"}
                continue
            ...
            nombre = first_line(get_single_cell(spreadsheet_id, "Booking ES", "G24"))
            nombre_safe = safe_filename(nombre if nombre else "Sin nombre")
            pdf_name = safe_filename(f"CVC Agencias {nombre_safe} {locator_clean}.pdf")

            yield {"type": "status", "msg": f"ðŸ“„ Generando PDF de la hoja **CVC Agencias**..."}
            pdf_bytes = export_sheet_pdf_bytes(spreadsheet_id, titles["CVC Agencias"])
...
st.markdown(
    ...
    * { box-sizing: border-box; }
    ...
    .cvcfit-log-line:last-child, .cvcagencias-log-line:last-child {
        margin-bottom: 0;
    }
    ...
    @media (max-width: 1600px) {
        .agency-grid, .cvcfit-grid, .cvcagencias-grid { grid-template-columns: 1fr; }
    }
...
with col9:
    st.markdown(
        """
        <div class="action-box card-cvcagencias">
            <div class="action-top">
                <div class="action-icon">ðŸ·ï¸</div>
                <div class="action-text">
                    <div class="action-title">CVC Agencias</div>
                    <div class="action-title-en">CVC Agencies</div>
                    <div class="action-desc">Buscar localizador en Booking ES!G11 y descargar PDF de la hoja CVC Agencias</div>
                    <div class="action-desc-en">Find locator in Booking ES!G11 and download the CVC Agencias sheet as PDF</div>
                </div>
            </div>
            <div class="action-button-wrap">
        """,
    ...
    if st.button("Abrir CVC Agencias", key="btncvcagenciasopen"):
        open_panel("cvcagencias")
        st.rerun()
    st.markdown("</div></div>", unsafe_allow_html=True)
...
# *************** 13B. SEGUNDA FILA **************************
# ************************************************************
row2col1, row2col2, row2col3, row2col4, row2col5, row2col6, row2col7, row2col8, row2col9 = st.columns(9, gap="medium")

with row2col1:
    st.markdown(
        """
        <div class="action-box card-nextcard">
            <div class="action-top">
                <div class="action-icon">ðŸš€</div>
                <div class="action-text">
                    <div class="action-title">NextCard</div>
                    <div class="action-title-en">NextCard</div>
                    <div class="action-desc">Tarjeta reservada para un uso futuro</div>
                    <div class="action-desc-en">Reserved card for future use</div>
                </div>
            </div>
            <div class="action-button-wrap">
        """,
    ...
    st.button("PrÃ³ximamente", key="btnnextcardfuture", disabled=True)
    st.markdown("</div></div>", unsafe_allow_html=True)
...
if st.session_state.get("opencvcfitform"):
    ...
    if st.button("Generar PDF CVC Fit", key="btncvcfitaction", disabled=not locator):
        ...
        try:
            ...
            for event in gen:
                ...
                if etype == "progress":
                    ...
                    progress_bar.progress(
                        pct,
                        text=f"Revisando {event['current']}/{event['total']} Â· {event['file']}"
                    )
                    status_box.markdown(
                    ...
                    result = event
                    progress_bar.progress(1.0, text="âœ… PDF generado correctamente")
                    status_box.empty()
            ...
            if result:
                ...
                st.error("BÃºsqueda finalizada sin coincidencias.")
    ...
    if log_lines_saved:
        ...
        st.markdown("**Log de la bÃºsqueda:**")
        st.markdown(
    ...
    if result:
        ...
        with colb:
            st.markdown(
                f'<a class="done-link" href="{result["spreadsheet_url"]}" target="_blank">ðŸ“Š Abrir hoja origen en Drive</a>',
                unsafe_allow_html=True,
            )
...
if st.session_state.get("opencvcagenciasform"):
    ...
    if st.button("Generar PDF CVC Agencias", key="btncvcagenciasaction", disabled=not locator):
        st.session_state["cvcagencias_result"] = None
        st.session_state["cvcagencias_log"] = []
        ...
        try:
            gen = build_cvc_agencias_pdf_from_locator(locator)
            result = None
            ...
            for event in gen:
                ...
                if etype == "progress":
                    ...
                    status_box.markdown(
                        f"<div class='cvcagencias-log-line'>{event['msg']}</div>",
                        unsafe_allow_html=True,
                    )
                    ...
                    log_box.markdown(
                        "<br>".join(
                            f"<div class='cvcagencias-log-line'>{l}</div>" for l in log_lines[-12:]
                        ),
                        unsafe_allow_html=True,
                    )
                    ...
                    log_box.markdown(
                        "<br>".join(
                            f"<div class='cvcagencias-log-line' style='color:#9BA5B7;'>{l}</div>"
                            for l in log_lines[-12:]
                        ),
                        unsafe_allow_html=True,
                    )
            ...
            if result:
                st.session_state["cvcagencias_result"] = result
                st.session_state["cvcagencias_log"] = log_lines
            else:
                st.error("BÃºsqueda finalizada sin coincidencias.")
    ...
    if log_lines_saved:
        ...
        st.markdown("**Log de la bÃºsqueda:**")
        st.markdown(
            "<br>".join(f"<div class='cvcagencias-log-line'>{l}</div>" for l in log_lines_saved),
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if result:
        st.markdown('<div class="cvcagencias-card" style="margin-top:1rem;"><div class="cvcagencias-grid">', unsafe_allow_html=True)
        fields = [
            ("Localizador", result["locator"]),
            ("Nombre pasajero", result["nombre"]),
            ("Spreadsheet origen", result["spreadsheet_name"]),
        ...
        for label, value in fields:
            st.markdown(
                f"""
                <div>
                    <div class="cvcagencias-item-label">{label}</div>
                    <div class="cvcagencias-item-value">{value if value not in [None, ''] else '-'}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
st.markdown(
    ...
    <div class="portal-footer">
        <span class="footer-text">Panel de Control Â· Control Panel Â· v4.4.0</span>
