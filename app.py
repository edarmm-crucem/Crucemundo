...
# Carpeta destino para crear sesiones
FOLDER_ID = "11TP9aDv3ss5PWjeNsbr6WQ3mUS9ioEvm"

# Carpeta raíz general
...
def clear_process_state():
    for k in [
        "confirmstate", "sessiontype", "templateid", "nombrecopia",
        "copyurl", "processtitle", "step1_ok", "step2_ok", "step3_ok"
    ]:
        st.session_state.pop(k, None)
    st.session_state["confirmstate"] = "idle"
    st.session_state["sessiontype"] = ""
    st.session_state["templateid"] = ""
    ...
    st.session_state["step3_ok"] = False
...
def iniciar_proceso(sessiontype, templateid, prefixname, processtitle):
    ...
    st.session_state["confirmstate"] = "pending"
    st.session_state["sessiontype"] = sessiontype
    st.session_state["templateid"] = templateid
    ...
    st.session_state["step2_ok"] = False
    st.session_state["step3_ok"] = False
    st.session_state.pop(f"opened_{nombrecopia}", None)

    st.rerun()


def ejecutar_copia_sesion():
    sessiontype = st.session_state.get("sessiontype")
    templateid = st.session_state.get("templateid")
    nombrecopia = st.session_state.get("nombrecopia")

    if not sessiontype or not templateid or not nombrecopia:
        raise Exception("Faltan datos para crear la sesión.")
    ...
    descripcion = (
        f"Sesión creada para {st.session_state.get('displayname', 'Sin usuario')} | "
        f"Tipo: {sessiontype} | "
...
def build_cvc_fit_doc(data):
    ...
    add_line("Lugar y fecha", align=WD_ALIGN_PARAGRAPH.RIGHT)
    ...
    if data["dinerotext"]:
        for linea in data["dinerotext"].splitlines():
            add_line(linea)
    else:
        add_line("Sin detalle extraído del rango G33:R53")
    add_line(f"Total: {data['total']}")
    add_line(f"Fecha límite y/o calendario de pago del importe pendiente: {data['fechalimitepagostr']}")
...
def build_cvcfit_from_locator(locator):
    ...
    if locatorinfo["locator"] not in sheettitles:
        raise Exception(f"No existe una pestaña con nombre {locatorinfo['locator']} dentro del spreadsheet.")
    ...
    fechasalida = locatorinfo["fecha_salida"]
    fechalimitepago = locatorinfo["fecha_limite_pago"]

    filename = safe_filename(
        f"CVC Fit - {apellidos} {nombre} - {locatorinfo['boat_name']} - salida {fechasalida.strftime('%d-%m-%Y')}.docx"
    )

    payload = {
        "locator": locatorinfo["locator"],
        "boatname": locatorinfo["boat_name"],
        "spreadsheetid": spreadsheetid,
    ...
    if confirmstate in ["idle", "done"]:
        if st.button("Crear Sesión ES", key="btncreares"):
            iniciar_proceso(
                ...
                TEMPLATE_ID_ES,
                "MASTER",
                "Estado del Proceso / Process Status · Crear Sesión MASTER"
            )
    else:
...
if st.session_state.get("opencvcfitform"):
    ...
    if st.button("Generar CVC Fit", key="btncvcfitaction", disabled=not locator):
        try:
            payload = build_cvcfit_from_locator(locator)
            docio = build_cvc_fit_doc(payload)
...
if confirmstate in ["pending", "step1", "step2", "step3", "done"]:
    st.markdown('<div class="panel-inline" style="max-width:520px;">', unsafe_allow_html=True)
    st.markdown(f"### {processtitle}")

    step1_state = "done" if st.session_state.get("step1_ok") else ("active" if confirmstate in ["pending", "step1"] else "wait")
    step2_state = "done" if st.session_state.get("step2_ok") else ("active" if confirmstate == "step2" else "wait")
    step3_state = "done" if st.session_state.get("step3_ok") else ("active" if confirmstate == "step3" else "wait")

    render_step(
        "Plantilla localizada",
        "Se comprueba que la plantilla existe y está accesible.",
        step1_state
    )
    render_step(
        "Copia creada en Drive",
        savedname or "Creando archivo de sesión...",
        step2_state
    )
    render_step(
        "Sesión lista",
        "Archivo preparado y enlace final generado.",
        step3_state
    )

    if confirmstate == "pending":
        try:
            ejecutar_copia_sesion()
            st.rerun()
        except Exception as e:
            st.session_state["confirmstate"] = "idle"
            st.error(f"No se pudo crear la sesión: {e}")

    if confirmstate == "done" and savedurl:
        st.markdown(
            f"""
