...
if _modo("Inicio"):
    ...
    # ── Acceso rápido a Informe Cabinas ──────────────────────────
    st.markdown("#### 🚀 Acceso rápido <span style='font-size:0.65em;font-style:italic;color:#9CA3AF;'>Quick access</span>", unsafe_allow_html=True)

    if not salidas:
        st.info("No hay salidas creadas todavía. / *No departures created yet.*")
    else:
        col_quick, col_info = st.columns([1, 2])

        with col_quick:
            salida_quick = st.selectbox(
                "Selecciona una salida / *Select a departure*",
                salidas,
                key="inicio_salida_quick"
            )
            tipo_quick = st.selectbox(
                "Filtro / *Filter*",
                ["Todas las cabinas", "Solo VENDIDAS", "Solo RESERVAS", "Solo LIBRES"],
                key="inicio_tipo_quick"
            )

        with col_info:
            if salida_quick:
                datos_quick = getdatossalida(salida_quick)
                total = len(datos_quick)
                vendidas = sum(1 for d in datos_quick if d.get("estado") == "VENDIDA")
                reservas = sum(1 for d in datos_quick if d.get("estado") == "RESERVA")
                libres   = sum(1 for d in datos_quick if d.get("estado") == "LIBRE")

                st.markdown(f"""
                <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:10px;padding:1rem 1.2rem;margin-top:0.3rem;">
                    <div style="font-size:0.8rem;font-weight:700;color:#374151;margin-bottom:0.6rem;">
                        📋 Resumen salida <strong>{salida_quick}</strong>
                        <span style="font-size:0.68rem;font-style:italic;color:#9CA3AF;"> / Departure summary</span>
                    </div>
                    <div style="display:flex;gap:1.2rem;flex-wrap:wrap;">
                        <div style="text-align:center;">
                            <div style="font-size:1.4rem;font-weight:900;color:#1E3A8A;">{total}</div>
                            <div style="font-size:0.65rem;color:#6B7280;text-transform:uppercase;letter-spacing:0.05em;">Total</div>
                        </div>
                        <div style="text-align:center;">
                            <div style="font-size:1.4rem;font-weight:900;color:#B91C1C;">{vendidas}</div>
                            <div style="font-size:0.65rem;color:#6B7280;text-transform:uppercase;letter-spacing:0.05em;">Vendidas / Sold</div>
                        </div>
                        <div style="text-align:center;">
                            <div style="font-size:1.4rem;font-weight:900;color:#D97706;">{reservas}</div>
                            <div style="font-size:0.65rem;color:#6B7280;text-transform:uppercase;letter-spacing:0.05em;">Reservas / On Hold</div>
                        </div>
                        <div style="text-align:center;">
                            <div style="font-size:1.4rem;font-weight:900;color:#15803D;">{libres}</div>
                            <div style="font-size:0.65rem;color:#6B7280;text-transform:uppercase;letter-spacing:0.05em;">Libres / Free</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        if st.button("🛏️ Ver Informe de Cabinas / *Open Cabin Report*", type="primary"):
            # Genera la tabla inline sin cambiar de modo
            st.markdown("---")
            st.markdown(
                f"### 🛏️ Informe de Cabinas — Salida {salida_quick} "
                "<span style='font-size:0.6em;font-style:italic;color:#9CA3AF;'>Cabin Report</span>",
                unsafe_allow_html=True
            )

            datos_inicio = getdatossalida(salida_quick)
            datos_filtrados_inicio = [
                d for d in datos_inicio if
                (tipo_quick == "Todas las cabinas") or
                (tipo_quick == "Solo VENDIDAS"  and d.get("estado") == "VENDIDA") or
                (tipo_quick == "Solo RESERVAS"  and d.get("estado") == "RESERVA") or
                (tipo_quick == "Solo LIBRES"    and d.get("estado") == "LIBRE")
            ]
            ...
            t += th_i("Cabina", "Cabin") + th_i("Agencia", "Agency") + th_i("PAX", "PAX") + th_i("Localizador", "Ref") + th_i("Notas", "Notes")
            t += '</tr></thead><tbody>'

            for d in sorted(datos_filtrados_inicio, key=lambda x: int(re.sub(r"[^0-9]", "", x.get("cabina", "0")) or 0)):
